...
namespace Engine
{
	...
	public struct Matrix4x4
	{
		...
		public static Matrix4x4 CreateFrustumMatrix(float fov, float aspect, float near, float far)
		{
			...
			projectionMatrix.column_3.z = -1f;
			projectionMatrix.column_2.w = -((2f * near * far) / frustum_length);
			projectionMatrix.column_3.w = 0f;

			return projectionMatrix.GetTransponed();
		}
		...
		{
			Matrix4x4 norm = new Matrix4x4();
			norm.column_0 = column_0.normalized;
			norm.column_1 = column_1.normalized;
			norm.column_2 = column_2.normalized;

			float tr = Mathf.Sqrt(1f + norm.column_0.x + norm.column_1.y + norm.column_2.z) * 0.5f;
			float qx, qy, qz, qw;
			if (tr > 0)
			{
				...
				float sd = 1f / s;
				qw = 0.25f * s;
				qx = (norm.column_2.y - norm.column_1.z) * sd;
				qy = (norm.column_0.z - norm.column_2.x) * sd;
				qz = (norm.column_1.x - norm.column_0.y) * sd;
			}
			else if ((norm.column_0.x > norm.column_1.y) & (norm.column_0.x > norm.column_2.z))
			{
				float s = Mathf.Sqrt(1.0f + norm.column_0.x - norm.column_1.y - norm.column_2.z) * 2;
				float sd = 1f / s;
				qw = (norm.column_2.y - norm.column_1.z) * sd;
				qx = 0.25f * s;
				qy = (norm.column_0.y + norm.column_1.x) * sd;
				qz = (norm.column_0.z + norm.column_2.x) * sd;
			}
			else if (norm.column_1.y > norm.column_2.z)
			{
				float s = Mathf.Sqrt(1.0f + norm.column_1.y - norm.column_0.x - norm.column_2.z) * 2;
				float sd = 1f / s;
				qw = (norm.column_0.z - norm.column_2.x) * sd;
				qx = (norm.column_0.y + norm.column_1.x) * sd;
				qy = 0.25f * s;
				qz = (norm.column_1.z + norm.column_2.y) * sd;
			}
			else
			{
				float s = Mathf.Sqrt(1.0f + norm.column_2.z - norm.column_0.x - norm.column_1.y) * 2;
				float sd = 1f / s;
				qw = (norm.column_1.x - norm.column_0.y) * sd;
				qx = (norm.column_0.z + norm.column_2.x) * sd;
				qy = (norm.column_1.z + norm.column_2.y) * sd;
				qz = 0.25f * s;
			}
